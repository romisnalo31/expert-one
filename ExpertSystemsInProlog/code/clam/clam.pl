...
pretty(av(A,yes),[A]) :- !.
pretty(\+ av(A,yes), [not,A]) :- !.
pretty(av(A,no),[not,A]) :- !.
pretty(\+ av(A,V),[not,A,is,V]).
...
bugxx(cf_model(X)) :- write(cf_model(X)),nl,!.
bugxx(ghoul(X)):- write(ghoul(X)),nl,!.
bugxx(askable(A,_,_,_)):- write('askable '),write(A),nl,!.
bugxx(output(A,V,PL)):- write('output '),write(V),nl,!.
bugxx(rule(N,_,_)):- write('rule '),write(N),nl,!.
bugxx(X) :- write(X),nl.
...
phraz(\+ av(Attr,yes)) --> [not,Attr].
phraz(\+ av(Attr,yes)) --> [not,a,Attr].
phraz(\+ av(Attr,yes)) --> [not,an,Attr].
phraz(\+ av(Attr,Val)) --> [not,Attr,is,Val].
phraz(\+ av(Attr,Val)) --> [not,Attr,are,Val].
